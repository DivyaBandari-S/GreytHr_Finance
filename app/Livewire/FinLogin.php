...
class FinLogin extends Component
{
    ...
    public function finLogin()
    {
        ...
        try {
            ...
            if (Auth::guard('fin')->attempt(['fi_emp_id' => $this->form['emp_id'], 'password' => $this->form['password']])) {
                return redirect()->route('hello');
            } elseif (Auth::guard('fin')->attempt(['email' => $this->form['emp_id'], 'password' => $this->form['password']])) {
                return redirect()->route('hello');
            }  else {
    ...
    public function verifyEmailAndDOB()
    {
        ...
        try {
            ...
            $userInHR = Finance::where(function ($query) use ($email) {
                $query->where('email', $email)
                    ->orWhere('company_email', $email);
            })->where('date_of_birth', $this->dob)->first();
    ...
    {
        return view('livewire.fin-login');
    }
}
